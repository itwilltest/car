...
public class CarOrderBean {
	...
	private int orderid; // 자동차 렌트(대여) 주문id저장
	// 렌트할 차량 차번호 저장 : 차번호로 조인하여 검색하기 위하여
	private int carno;
	private int carqty;// 렌트차량 대여 수량 저장
	private int carreserveday;// 대여기간 저장
	private String carbegindate;// 자동차를 렌트(대여)할 시작날짜 저장 , 대여날짜
	private int carins;// 렌트시 보험적용 일수 저장
	private int carwifi;// 렌트시 무선wifi 적용 일수 저장
	private int carnave;// 렌트시 네비게이션 적용여부 저장(무료로 적용하면 1, 미적용이면 0)
	private int carbabyseat;// 렌트시 베이비 시트 적용 일수 저장
	private String memberphone;// 비회원으로 렌트할 사람의 폰번호 저장
